<?php

if ( function_exists( 'add_image_size' ) ) {
add_image_size ('attorneysmainbig-thamb', 390, 437, true);
add_image_size ('attorneysmainlitle-thamb', 40, 45, true);

add_image_size( 'testimonials-thumb', 130, 130, true ); // 300 в ширину и без ограничения в высоту
add_image_size ('practis-thambnail', 600, 300, true);
//	add_image_size( 'homepage-thumb', 220, 180, true ); // Кадрирование изображения

add_image_size ('mainpage-thamb', 360, 256, true);
add_image_size ('singlepage-thamb', 1920, 1000, true);


add_image_size ('mainblogpage-thamb', 350, 270, true);

}


?>